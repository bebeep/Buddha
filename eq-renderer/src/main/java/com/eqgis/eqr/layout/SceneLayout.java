...
public class SceneLayout extends FrameLayout{
    ...
    // 触摸旋转相关字段
    private volatile Node touchRotationTarget;
    private float lastTouchX = 0f;
    private float lastTouchY = 0f;
    private boolean isUserInteracting = false;
    private float currentRotationX = 0f;
    private float currentRotationY = 0f;
    private float touchRotationSensitivity = 0.5f;
    private volatile boolean touchRotationEnabled = false;
    private Quaternion initialRotationOffset = Quaternion.identity();
    private boolean rotateHorizontalEnabled = true;
    private boolean rotateVerticalEnabled = true;

    // 自动旋转相关字段
    private Node autoRotateTarget;
    private float autoRotateSpeed = 45f; // 每秒旋转角度
    private Vector3 autoRotateAxis = Vector3.up();
    private boolean autoRotateEnabled = false;
    private Scene.OnUpdateListener autoRotateListener;
...
//                setSkybox(skybox);
    ...
    /**
     * 启用触摸旋转控制
     * @param target 要旋转的目标节点
     * @param sensitivity 每像素灵敏度，建议值 0.3f-1.0f
     */
    public void enableTouchRotation(Node target, float sensitivity) {
        enableTouchRotation(target, sensitivity, true, true);
    }
     ...
     * @param target 要旋转的目标节点
     * @param sensitivity 每像素灵敏度，建议值 0.3f-1.0f
     * @param enableHorizontal 是否启用水平（Y轴）旋转
     * @param enableVertical 是否启用垂直（X轴）旋转
     */
    public void enableTouchRotation(Node target, float sensitivity, boolean enableHorizontal, boolean enableVertical) {
        this.touchRotationTarget = target;
        this.touchRotationSensitivity = sensitivity;
        this.touchRotationEnabled = true;
        this.rotateHorizontalEnabled = enableHorizontal;
        this.rotateVerticalEnabled = enableVertical;
        this.currentRotationX = 0f;
        this.currentRotationY = 0f;
        setClickable(true);
        Log.d("SceneLayout", "enableTouchRotation: target=" + target + ", h=" + enableHorizontal + ", v=" + enableVertical);
        applyTouchRotation();
    }
     ...
     * @param target 要旋转的目标节点
     */
    public void enableTouchRotation(Node target) {
        enableTouchRotation(target, 0.5f);
    }

    /**
     * 禁用触摸旋转
     */
    public void disableTouchRotation() {
        this.touchRotationEnabled = false;
        this.touchRotationTarget = null;
        setClickable(false);
    }

    /**
     * 设置初始旋转偏移量，触摸旋转会在此基础上叠加
     * @param rotation 触摸旋转前的基础旋转
     */
    public void setInitialRotationOffset(Quaternion rotation) {
        this.initialRotationOffset = rotation;
        applyTouchRotation();
    }
     ...
     * 将当前旋转应用到目标节点
     */
    private void applyTouchRotation() {
        if (touchRotationTarget == null) return;
        Quaternion rotationY = new Quaternion(Vector3.up(), currentRotationY);
        Quaternion rotationX = new Quaternion(Vector3.right(), currentRotationX);
        touchRotationTarget.setLocalRotation(Quaternion.multiply(initialRotationOffset, Quaternion.multiply(rotationY, rotationX)));
    }
     ...
     * 启动自动旋转
     * @param target 要旋转的目标节点
     * @param degreesPerSecond 旋转速度（度/秒） +朝右 -朝左
     * @param axis 旋转轴（如 Vector3.up() 水平旋转，Vector3.right() 垂直旋转）
     */
    public void startAutoRotation(Node target, float degreesPerSecond, Vector3 axis) {
        stopAutoRotation();
        this.autoRotateTarget = target;
        this.autoRotateSpeed = degreesPerSecond;
        this.autoRotateAxis = axis;
        this.autoRotateEnabled = true;

        autoRotateListener = new Scene.OnUpdateListener() {
            @Override
            public void onUpdate(FrameTime frameTime) {
                if (!autoRotateEnabled || autoRotateTarget == null) return;
                float dt = frameTime.getDeltaSeconds();
                if (dt > 0 && dt < 1f) {
                    float angle = autoRotateSpeed * dt;
                    Quaternion delta = new Quaternion(autoRotateAxis, angle);
                    autoRotateTarget.setLocalRotation(Quaternion.multiply(delta, autoRotateTarget.getLocalRotation()));
                }
            }
        };
        sceneView.getScene().addOnUpdateListener(autoRotateListener);
    }
     ...
     * @param target 要旋转的目标节点
     */
    public void startAutoRotation(Node target) {
        startAutoRotation(target, 45f, Vector3.up());
    }
    ...
    public void stopAutoRotation() {
        ...
        if (autoRotateListener != null && sceneView != null && sceneView.getScene() != null) {
            sceneView.getScene().removeOnUpdateListener(autoRotateListener);
    ...
    public boolean onInterceptTouchEvent(MotionEvent ev) {
        if (touchRotationEnabled && touchRotationTarget != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    lastTouchX = ev.getX();
                    lastTouchY = ev.getY();
                    isUserInteracting = true;
                    Log.d("SceneLayout", "onInterceptTouchEvent DOWN: intercepted=true");
                    return true;
            }
        }
        ...
        if (touchRotationEnabled && touchRotationTarget != null) {
            switch (ev.getAction()) {
                case MotionEvent.ACTION_DOWN:
                    lastTouchX = ev.getX();
                    lastTouchY = ev.getY();
                    Log.d("SceneLayout", "onTouchEvent DOWN: returning true");
                    return true;
                case MotionEvent.ACTION_MOVE:
                    float deltaX = ev.getX() - lastTouchX;
                    float deltaY = ev.getY() - lastTouchY;
                    if (rotateHorizontalEnabled) {
                        currentRotationY += deltaX * touchRotationSensitivity;
                    }
                    if (rotateVerticalEnabled) {
                        currentRotationX -= deltaY * touchRotationSensitivity;
                        currentRotationX = Math.max(-90f, Math.min(90f, currentRotationX));
                    }
                    applyTouchRotation();
                    lastTouchX = ev.getX();
                    lastTouchY = ev.getY();
