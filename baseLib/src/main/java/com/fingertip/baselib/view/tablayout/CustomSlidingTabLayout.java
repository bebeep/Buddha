...
public class CustomSlidingTabLayout extends HorizontalScrollView implements ViewPager.OnPageChangeListener {
    ...
    private int[] mIndicatorColors;//支持渐变色
    ...
    private int mIndicatorHorizontal;
    ...
    private void obtainAttributes(Context context, AttributeSet attrs) {
        ...
        mIndicatorHorizontal = ta.getInt(R.styleable.SlidingTabLayout_tl_indicator_horizontal, Gravity.CENTER_HORIZONTAL);
...
//            v.setPadding((int) mTabPadding, v.getPaddingTop(), (int) mTabPadding, v.getPaddingBottom());
    ...
    private void calcIndicatorRect() {
        ...
        if (mIndicatorWidth < 0) {   //indicatorWidth小于0时,原jpardogo's PagerSlidingTabStrip
            ...
            float indicatorLeft = currentTabView.getLeft() + (mIndicatorHorizontal==Gravity.CENTER_HORIZONTAL?(currentTabView.getWidth() - mIndicatorWidth) / 2:mTabPadding);
            if (this.mCurrentTab < mTabCount - 1 ) {
                View nextTab = mTabsContainer.getChildAt(this.mCurrentTab + 1);
                indicatorLeft = indicatorLeft + mCurrentPositionOffset * (mIndicatorHorizontal==Gravity.CENTER_HORIZONTAL?(currentTabView.getWidth() / 2 + nextTab.getWidth() / 2):currentTabView.getWidth());
            }
