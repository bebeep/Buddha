 ...
 *
 */
class MessageReplyAdapter(context: Context, val onItemClick:(position:Int)->Unit):
    TopRcAdapter<String, TopRcAdapter.TopRcViewHolder>(context) {

    override fun initLayoutId(viewType: Int) = R.layout.item_message_reply
    ...
    override fun onBindViewHolder(holder: TopRcViewHolder, position: Int) {
        get(position)?.let{
            ...
            holder.itemView.findViewById<View>(R.id.cl_reply_parent).setOnClickListener { view-> onItemClick(position) }
            holder.itemView.findViewById<View>(R.id.cl_reply_parent).setOnLongClickListener { view->
