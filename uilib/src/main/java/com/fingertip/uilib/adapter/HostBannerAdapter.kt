 ...
 * 通用的bannerAdapter
 */
class HostBannerAdapter(val context: Context, list:List<BannerEntity>, val onClick:(viewId:Int, pos:Int)->Unit):BannerAdapter<BannerEntity,HostBannerAdapter.BannerViewAdapter>(list) {



    override fun onCreateHolder(parent: ViewGroup?, viewType: Int): BannerViewAdapter {
        val view = LayoutInflater.from(context).inflate(R.layout.item_host_banner, parent, false)
