...
class MomentAdapter(context: Context,val onItemClick:(entity:MomentEntity)->Unit) : TopRcAdapter<MomentEntity, TopRcAdapter.TopRcViewHolder>(context) {
    ...
    override fun onBindViewHolder(holder: TopRcViewHolder, position: Int) {
        ...
        get(position)?.let {
            holder.itemView.findViewById<View>(R.id.v_divider).visibility = if (position == mlist.size-1) View.GONE else View.VISIBLE
            holder.itemView.findViewById<androidx.recyclerview.widget.RecyclerView>(R.id.rv_photos).visibility = if (it.momentType == 2  && it.images.size > 0) View.VISIBLE else View.GONE
            if (it.momentType == 2  && it.images.size > 0 ){//显示图片集合
                ...
                val rvPhotos = holder.itemView.findViewById<androidx.recyclerview.widget.RecyclerView>(R.id.rv_photos)
                rvPhotos.layoutManager = GridLayoutManager(context,colums)
