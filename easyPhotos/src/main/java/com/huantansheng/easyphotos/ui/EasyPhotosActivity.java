...
import com.huantansheng.easyphotos.ui.dialog.CameraTypeDialog;
...
public class EasyPhotosActivity extends AppCompatActivity implements AlbumItemsAdapter.OnClickListener, PhotosAdapter.OnClickListener, AdListener, View.OnClickListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        TopApplication.Companion.addActivity("EasyPhotosActivity",this);
        initSomeViews();
...
//        startActivity(new Intent(this,CameraActivity.class)); //自定义相机


        takeCameraAndVideo(requestCode);
    }


    private CameraTypeDialog cameraTypeDialog;
    private void takeCameraAndVideo(final int requestCode){
        if (cameraTypeDialog == null){
            cameraTypeDialog = new CameraTypeDialog(this);
            cameraTypeDialog.setOnButtonClickListener(new CameraTypeDialog.OnButtonClickListener() {
                @Override
                public void onClick(View view) {
                    cameraTypeDialog.cancel();
                    isVideo = false;
        ...
        cameraTypeDialog.show();


//        startActivityForResult(new Intent(this,CameraActivity.class),555);
    }
    ...
    private boolean isVideo = false;//是否为录制视频

    private int cameraRequestCode = 0;
    private void toAndroidCamera(int requestCode) {
        Intent cameraIntent = new Intent(isVideo?MediaStore.ACTION_VIDEO_CAPTURE:MediaStore.ACTION_IMAGE_CAPTURE);
...
//            }
            ...
            if (mTempImageFile != null && mTempImageFile.isFile()) {
                ...
                if (!isVideo)cameraIntent.putExtra(MediaStore.EXTRA_OUTPUT, imageUri);//将拍取的照片保存到指定URI
    ...
    public void finishSystemCamera(){
        try {
            finishActivity(cameraRequestCode);
        }catch (Exception e){
...
//        File dir = null;
        ...
        try {
            mTempImageFile = File.createTempFile(isVideo?"VIDEO":"IMG", isVideo?".mp4":".jpg", dir);
        } catch (IOException e) {
            e.printStackTrace();
            mTempImageFile = null;
        }
...
//        super.onActivityResult(requestCode, resultCode, data);
        ...
        switch (resultCode) {
            case RESULT_OK:
                if (requestCode == 555 && data!=null){//拍照/录视频回调
                    File imageFile = (File) data.getSerializableExtra("imageFile");
                    File videoFile = (File) data.getSerializableExtra("videoFile");
                    if (videoFile!=null && videoFile.exists() && videoFile.length()>0){
                        isVideo = true;
                        mTempImageFile = videoFile;
                        onCameraResult();
                    }
                    if (imageFile!=null && imageFile.exists() && imageFile.length()>0){
                        isVideo = false;
                        mTempImageFile = imageFile;
                        onCameraResult();
                    }
                    return;
                }
                if (Code.REQUEST_CAMERA == requestCode) {
                    LogUtil.e("isQ:"+isQ+"|photo:"+(data==null?null:data.getData()));
                    if (isVideo && data!=null) {
                        photoUri = data.getData();
                        mTempImageFile = new File(FileUtils.getPath(this,photoUri));
                    }
//                    if (isQ) {
//                        onCameraResultForQ();
//                        return;
//                    }
                    ...
                    if (mTempImageFile == null || !mTempImageFile.isFile()) {
                        throw new RuntimeException("Photo not exists");
                    }
                    onCameraResult();
                    return;
                }
                ...
                if (Code.REQUEST_PUZZLE_SELECTOR == requestCode) {
                    PhotoEntity puzzlePhoto = data.getParcelableExtra(EasyPhotos.RESULT_PHOTOS);
    ...
    private void addNewPhoto(PhotoEntity photo) {
    ...
    private PhotoEntity getPhoto(Uri uri) {
        PhotoEntity p = null;
        String path;
        String name;
        long dateTime;
        ...
        if (cursor.moveToFirst()) {
            ...
            p = new PhotoEntity(name, uri, path, dateTime, width, height, orientation, size, 0, type);
        }
        cursor.close();
    ...
    private void onCameraResultForQ() {
        ...
        new Thread(new Runnable() {
            @Override
            public void run() {
                final PhotoEntity photo = getPhoto(photoUri);
                LogUtil.e("photo Q:"+photo);
                if (photo == null) {
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                SimpleDateFormat dateFormat = new SimpleDateFormat("yyyyMMdd_HH_mm_ss",
                        Locale.getDefault());
                String imageName = isVideo?"VIDEO_%s.mp4":"IMG_%s.jpg";
                ...
                final PhotoEntity photo = new PhotoEntity(mTempImageFile.getName(), uri,
                        ...
                        isVideo?"video/mp4":options.outMimeType);

                LogUtil.e("photo："+photo);
                LogUtil.e("photo："+(mTempImageFile.exists())+"|"+mTempImageFile.length());

                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        if (photo.type.contains("video")){//视频不预览，直接跳转到裁剪页面去预览
                            resultList.add(photo);
                            takeVideoBack(photo.path);
                            return;
                        }
                        if (Setting.onlyStartCamera || albumModel.getAlbumItems().isEmpty()) {
                            Intent data = new Intent();
    ...
    private void initView() {
        ...
        //针对文件类型进行排序
//        Collections.sort(photoList, new Comparator<Object>() {
//            @Override
//            public int compare(Object o1, Object o2) {
//                if (o1 instanceof Photo && o2 instanceof Photo){
////                    int i = 0;
////                    if (((Photo)o1).type.contains("video") || ((Photo)o2).type.contains("video")){
////                        return ((Photo)o1).time > ((Photo)o2).time?1:(((Photo)o1).time == ((Photo)o2).time?0:-1);
////                    }else if(((Photo)o1).type.contains("video") || ((Photo)o2).type.contains("video")) return 1;
////                    else if (!((Photo)o1).type.contains("video") && ((Photo)o2).type.contains("video")) return -1;
//                    return (((Photo)o1).type.contains("video") || ((Photo)o2).type.contains("video"))?1:0;
//                }
//                return 0;
//            }
//        });
        Log.e("photoList","photoList："+photoList);
...
//        }
            ...
            public void run() {
                ...
                try {
                    for (int i = 0; i < size; i++) {
                        PhotoEntity photo = Result.photos.get(i);
    ...
    private void shouldShowMenuDone() {
        ...
        if (Setting.complexSelector) {
            if (Setting.complexSingleType && !Result.photos.isEmpty()) {
//                String type = Result.getPhotoType(0);
//                if (type!=null && type.contains(Type.VIDEO)) {
...
//                }
    ...
    public void onPhotoClick(int position, int realPosition) {
        //视频直接跳转到编辑页面，图片跳转到预览页面

        PhotoEntity photo  = AlbumModel.instance.getCurrAlbumItemPhotos(currAlbumItemIndex).get(realPosition);
        LogUtil.e("photo:选中-》"+photo);
        if (photo.type.contains("video")){//视频不预览，直接跳转到裁剪页面去预览
            if (!photo.path.endsWith(".mp4") && !photo.path.endsWith(".MP4")){//不支持mp4以外的格式
                ToastUtil.INSTANCE.showMessage("Unsupported video type",Toast.LENGTH_SHORT);
                return;
            }
            takeVideoBack(photo.path);
            return;
     ...
     * @param path
     */
    private void takeVideoBack(String path){
        Intent intent = new Intent();
        intent.putExtra("path",path);
        intent.putExtra("operate",1);//1录制视频
        setResult(RESULT_OK,intent);
        finish();
    }
